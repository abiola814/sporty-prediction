...
class Updater:
    def __init__(self, current_season: int):
        ...
        token = getenv("TOKEN")
        self.url = "https://api.football-data.org/"
    ...
    async def get(url: str, headers: Optional[dict] = None):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.request("GET", url=url, headers=headers, ssl=False)
    ...
    def save_team_data_to_db(self):
        ...
        print(team_data, "kjfkjkkkkkkkkkkk")
    ...
    def build_all(
        ...
        try:
            ...
            self.set_raw_data(num_seasons, request_new := True)

        self.build_dataframes(num_seasons, display_tables)
        print("re", request_new)
        if request_new:
